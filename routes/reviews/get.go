...
func getBotReviews(d uapi.RouteData, r *http.Request) uapi.HttpResponse {
	...
	rows, err := state.Pool.Query(d.Context,
		"SELECT "+reviewColumns+" FROM reviews WHERE bot_id = $1 AND status = $2 ORDER BY created_at DESC LIMIT $3 OFFSET $4",
		botID, types.ReviewStatusPublished, limit, offset)
	if err != nil {
		return helpers.InternalError(err)
	}
