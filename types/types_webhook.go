...
// Webhook is an outbound event subscription registered against a target —
// a bot, a business, a project, or (with no schema change needed) whatever
// gets added next. TargetType/TargetID mirror the same polymorphic shape
// Report/ModerationAction already use.
type Webhook struct {
	ID              uuid.UUID  `db:"id" json:"id" description:"The webhook's ID"`
	TargetType      string     `db:"target_type" json:"target_type" description:"What this webhook is registered against: bot, business, project, ..."`
	TargetID        string     `db:"target_id" json:"target_id" description:"The ID of the target (a bot_id, or a business/project UUID)"`
...
// WebhookRevealed is a Webhook plus its signing secret in the clear —
// returned only right after creation or a secret rotation, the one time
// the caller can see it. The outer Secret field shadows the embedded
// Webhook's json:"-" one, so this is the only response shape that ever
// serializes it.
...
type WebhookCreate struct {
	TargetType string   `json:"target_type" validate:"required" msg:"A target_type is required" description:"What to register this webhook against: bot, business, project, ..."`
	TargetID   string   `json:"target_id" validate:"required" msg:"A target_id is required" description:"The ID of the target (a bot_id, or a business/project UUID)"`
...
// WebhookEvent is one entry in the webhooks package's fixed event catalog,
// for populating a subscription picker.
